...
Example:
  python3 load_generator.py --M 136 --N 30 --seeds 20 --alpha 5 --beta 5
"""
...
def send_request(device_id, seed_val, results, url, timeout, logger=None):
    """
    Sends one POST request and appends (seed, timestamp, device, entity, status, latency_ms)
    to results list.
...
def parse_args():
    ...
    p.add_argument("--key", type=str, default="12345", help="API key value for query param 'k'.")
...
def main():
    ...
    # Ensure URL has key parm
    if "?" in base_url:
        url = f"{base_url}&k={api_key}&i="
    else:
        url = f"{base_url}?k={api_key}&i="
                ...
                for sec, count in enumerate(allocs, start=1):
                    ...
                    with ThreadPoolExecutor(max_workers=max_workers) as ex:
                        ...
                        for _ in range(int(count)):
                            ...
                            futures.append(ex.submit(send_request, did, seed_val, batch, full_url, timeout, logger))
