...
**fog** tier of the multi-tier Digital-Twin Smart-Parking experiment.
...
| `randomized_load_test_scenario_seed_37.csv` | The actual ordered list used to drive the fog-tier run (seed = 37). One row per scenario, columns `M,t,alpha,beta`. |
...
design is **4 × 9 × 9 = 324** experiments; the `fog` deployment
...
|---|---|---|---|
| **A — Deployment type** | `mist`, `edge`, `fog`, `cloud` | **4** | Where the IoT Agent / Orion-LD / MongoDB stack lives. This folder is the *fog* slice. |
...
graph LR
    ...
    A1 -.->|"× 81 scenarios"| Mist["mist tier<br/>onGeneratorScripts/"]
    A2 -.->|"× 81 scenarios"| Edge["edge tier<br/>onGenScripts/ + onJetson/"]
    A3 -.->|"× 81 scenarios<br/>(this folder)"| Fog["fog tier<br/>onGenScripts/ + onCluster/"]
    A4 -.->|"× 81 scenarios"| Cloud["cloud tier"]
    B1 & B2 & B3 & B4 & B5 & B6 & B7 & B8 & B9 -.-> Fog
    C1 & C2 & C3 & C4 & C5 & C6 & C7 & C8 & C9 -.-> Fog
...
The 81 fog scenarios are not run in any particular
...
The output is a CSV with header `M,N,alpha,beta` containing the same 81
scenarios in a deterministic-per-seed order.

```bash
# Regenerate this exact list
./generate_scenarios.sh --seed 37
# → randomized_load_test_scenario_seed_37.csv  (the file in this folder)
...
The orchestrator (`../onGenScripts/fog_deploy_runner.sh`) is invoked
**once per row** of the CSV. For each row `(M, t, α, β)` it performs
the full pipeline against VM2 (system under test) and the IC Discovery
Lab cluster node (fog inference node) — `docker compose up` on both
sides, healthy wait, Mongo indices, service group, provision `M`
devices, verify, then launch `load_generator.py` with `--M M --N t
--seeds S --alpha α --beta β` on VM1 — collects Prometheus metrics, and
`scp`s the results back. The rows are walked in CSV order (top to
bottom), so **row 1 in `randomized_load_test_scenario_seed_37.csv` is
the first fog experiment that was actually run**.

The data path for the fog tier is one hop longer than for mist: VM1
POSTs the parking **image** (not a pre-computed count) to the cluster
node's FastAPI `/predict` endpoint, the cluster node runs YOLOv11m
inference on the GTX 1080, and the count is then forwarded to VM2's
IoT Agent in the same `{"occupied_spots": N}` body that mist uses. The
same `(M, t, α, β)` row therefore stresses one extra hop and the
YOLO/TensorRT inference latency, which is what makes the fog tier a
distinct cell of the factorial design.

The seed-37 list in this folder starts with:
...
i.e. the first three experiments are `(250, 60, 1, 5)`, `(136, 240, 10, 10)`
and `(136, 60, 100, 100)`.
...
- **This folder:** the *fog* slice. Drives the order in which the
  81 fog experiments are launched.
- **Randomization:** seeded shuffle (seed 37) so order is reproducible
...
- **Driver:** `generate_scenarios.sh --seed <N>` regenerates the CSV
  for any seed; the seed-37 list committed here is the one that was
  used to schedule the actual fog-tier measurement campaign.
