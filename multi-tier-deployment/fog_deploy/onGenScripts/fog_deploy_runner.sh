...
# Usage: ./fog_deploy_runner.sh [config_file]

# In this case, I run Tailscale in a container inside the Discovery Cluster, since I can't have control of the VM of the cluster.
# For security and admin stuff, makes sense. Therefore, all the commands related to start, stop containers, do some processing on the Cluster's VM
# is not possible, so I must do it manually that part. Although, I can run this container on other Cluster's nodes, so I can parallelize these tests.
# By the way, I'm using AuthKeys from Tailscale, that way I don't need to login and authenticate, it is easier this way.
# Conclusion, most parts that I automate before for the cluster, must be performed manually. Hence, they won't appear here.

set -euo pipefail
IFS=$'\n\t'

CONFIG_FILE="${1:-fog_deploy.conf}"
...
required_vars=(M N seed alpha beta Cluster_tailscale_domain_name VM_tailscale_domain_name VM_tailscale_IPv4 VM_username VM_password VM_dir LM_dir onVM_dir onGen_dir onInfra_dir)
...
DIRNAME="fog_deploy_test_M_${M}_N_${N}_seed_${seed}_alpha_${alpha}_beta_${beta}"
...
"${LM_dir}/${onGen_dir}/load_generator.py" \
  ...
  --url "${Cluster_tailscale_domain_name}:8000/predict" \
...
# ---------------- COLLECT & PROCESS LOGS IN DISCOVERY CLUSTER----------------
# It's gone
# ---------------- COLLECT & PROCESS LOGS IN DISCOVERY CLUSTER----------------


# ---------------- COPY LOGS BACK FROM DISCOVERY CLUSTER ----------------
# ---------------- COPY LOGS BACK FROM DISCOVERY CLUSTER ----------------
