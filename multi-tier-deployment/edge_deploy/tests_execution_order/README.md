...
**edge** tier of the multi-tier Digital-Twin Smart-Parking experiment.
...
| `randomized_load_test_scenario_seed_8.csv` | The actual ordered list used to drive the edge-tier run (seed = 8). One row per scenario, columns `M,t,alpha,beta`. |
...
design is **4 × 9 × 9 = 324** experiments; the `edge` deployment
...
|---|---|---|---|
| **A — Deployment type** | `mist`, `edge`, `fog`, `cloud` | **4** | Where the IoT Agent / Orion-LD / MongoDB stack lives. This folder is the *edge* slice. |
...
graph LR
    ...
    A2 -.->|"× 81 scenarios<br/>(this folder)"| Edge["edge tier<br/>onGenScripts/ + onJetson/"]
    A3 -.->|"× 81 scenarios"| Fog["fog tier"]
    A4 -.->|"× 81 scenarios"| Cloud["cloud tier"]
    B1 & B2 & B3 & B4 & B5 & B6 & B7 & B8 & B9 -.-> Edge
    C1 & C2 & C3 & C4 & C5 & C6 & C7 & C8 & C9 -.-> Edge
...
The 81 edge scenarios are not run in any particular
...
./generate_scenarios.sh --seed 8
# → randomized_load_test_scenario_seed_8.csv  (the file in this folder)
...
The orchestrator (`../onGenScripts/edge_deploy_runner.sh`) is invoked
**once per row** of the CSV. For each row `(M, t, α, β)` it performs
the full pipeline against VM2 (system under test) and the Jetson Nano
(edge inference node) — `docker compose up` on both sides, healthy wait,
Mongo indices, service group, provision `M` devices, verify, then launch
`load_generator.py` with `--M M --N t --seeds S --alpha α --beta β` on
VM1 — collects Prometheus metrics, and `scp`s the results back. The
rows are walked in CSV order (top to bottom), so **row 1 in
`randomized_load_test_scenario_seed_8.csv` is the first edge experiment
that was actually run**.

The data path for the edge tier is one hop longer than for mist: VM1
POSTs the parking **image** (not a pre-computed count) to the Jetson's
FastAPI `/predict` endpoint, the Jetson runs inference, and the count
is then forwarded to VM2's IoT Agent in the same `{"occupied_spots":
N}` body that mist uses. The same `(M, t, α, β)` row therefore stresses
one extra hop and the YOLO/TensorRT inference latency, which is what
makes the edge tier a distinct cell of the factorial design.

The seed-8 list in this folder starts with:
...
i.e. the first three experiments are `(136, 240, 1, 1)`, `(136, 120, 1, 1)`
and `(136, 30, 5, 5)`.
...
- **This folder:** the *edge* slice. Drives the order in which the
  81 edge experiments are launched.
- **Randomization:** seeded shuffle (seed 8) so order is reproducible
...
- **Driver:** `generate_scenarios.sh --seed <N>` regenerates the CSV
  for any seed; the seed-8 list committed here is the one that was
  used to schedule the actual edge-tier measurement campaign.
