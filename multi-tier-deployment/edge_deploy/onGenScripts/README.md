...
[edge deployment](../README.md) campaign. The shell script in here is the
main entry point: it brings the **Jetson Nano** inference service up,
brings the **VM2** FIWARE NGSI-LD stack up, runs the load generator that
sends raw parking images to the Jetson over Tailscale, then pulls
Prometheus metrics and IoT-Agent / Jetson container logs back for offline
analysis. All six phases are orchestrated by `edge_deploy_runner.sh`; the
other files in the folder are the pieces it composes.
...
```mermaid
flowchart TD
    A["edge_deploy_runner.sh<br/>(runs on VM1)"] --> B["Phase 1 — Jetson bring-up<br/>docker compose up + tegrastats logger"]
    B --> C["Phase 2 — VM2 bring-up<br/>docker compose up + healthy wait"]
    C --> D["Phase 3 — IoT provisioning<br/>0_healthy_waiting → 1_…indices → 2_…service_group → 3_…provision → 4_…verify"]
    D --> E["Phase 4 — Load generator<br/>load_generator.py → POST images to Jetson :8000/predict"]
    E --> F["Phase 5 — Prometheus metrics<br/>get_metrics_posttest.py → query_range"]
    F --> G["Phase 6 — Log collection<br/>IoT-Agent logs (VM2) + Jetson container logs + tegrastats"]
...
Main entry point. Usage: `./edge_deploy_runner.sh [config_file]` (defaults
to `./edge_deploy.conf`).
...
- Validates that every required variable is set: `M N seed alpha beta
  VM_tailscale_domain_name VM_tailscale_IPv4 Jetson_tailscale_domain_name
  Jetson_tailscale_IPv4 VM_username VM_password Jetson_username
  Jetson_password VM_dir Jetson_dir LM_dir onVM_dir onJetson_dir onGen_dir
  onInfra_dir`.
...
- Runs the six phases in order, over `sshpass` to the Jetson and to VM2,
  then `scp`s the per-target artefacts back into the per-run directory.

The phases correspond exactly to the mermaid diagram above. Logs from
the IoT-Agent container on VM2 are filtered with
`docker ps -a --format '{{.Names}}' | grep 'fiware-iot-agent'`; logs from
the Jetson container use `grep 'jetson-ml_container'`. If either match
fails, the runner prints a warning and continues.

### `edge_deploy.conf` / `edge_deploy.conf.example`

Shell-sourceable config. `edge_deploy.conf` is the real, gitignored
file with Tailscale IPs and SSH credentials; `edge_deploy.conf.example`
is the committed template.

```bash
# Template workflow (do NOT edit the .example in place)
cp edge_deploy.conf.example edge_deploy.conf
$EDITOR edge_deploy.conf
```
...
| Jetson Tailscale | `Jetson_tailscale_domain_name` | Tailscale MagicDNS name of the Jetson (load-gen target `<host>:8000/predict`). |
| Jetson Tailscale | `Jetson_tailscale_IPv4` | Tailscale IPv4 of the Jetson. |
| Jetson Tailscale | `Jetson_username` / `Jetson_password` | SSH credentials for the Jetson. |
| Paths | `VM_dir` | Repo root on VM2 (parent of `onVMScripts/`, `infra/`). |
| Paths | `Jetson_dir` | Repo root on the Jetson (parent of `onJetson/`). |
...
| Paths | `onJetson_dir` | Jetson-side scripts dir, relative to `Jetson_dir`. |
...
Jetson for every simulated device. CLI flags:
...
| `--url` | `http://localhost:7896/iot/json` | Target URL. **In the edge runner this is overridden to `<jetson>:8000/predict`.** |
| `--timeout` | `300` | Per-request timeout in seconds. |
| `--image` | required | Path to the image uploaded with every request. |

Behaviour worth knowing about for the edge deployment:
...
- The HTTP body is a **`multipart/form-data`** upload, not a JSON body.
  ...
  form field:
  ```text
  POST <jetson>:8000/predict
  Content-Type: multipart/form-data
  file=@test.jpg
  entity_id=urn:ngsi-ld:OffStreetParking:<device_id>
  ```
  This is what makes the FastAPI service on the Jetson able to forward
  the count back to the right IoT-Agent device downstream.
...
| `--prom-url` | `http://localhost:9090` | Prometheus base URL. **In the edge runner this is overridden to `<vm2_tailscale>:9090`.** |
...
- One `metrics_{M_x_N_x_seed_x_TR_x_alpha_x_beta_x}.csv` per response
  ...
  not part of the edge filename — the script falls back to a copy of
...
empty. Intended for one-off sanity checks on Jetson-side inference
...
cp edge_deploy.conf.example edge_deploy.conf
$EDITOR edge_deploy.conf            # fill in Tailscale + SSH credentials
...
- `raw_logs_jetson-ml_container_*.log-jetson` and the matching
  `processed_raw_logs_*.csv` — Jetson inference container logs and
  their processed form, collected over SSH from the Jetson.
- `raw_logs_jetson_tegrastats_*.log-jetson-tegrastats` and the
  matching `processed_raw_logs_jetson_tegrastats_*.csv` — Jetson
  `tegrastats` telemetry; the raw stream is collected on the Jetson but
  parsed into CSV on the load-generator host because the conversion
  is too slow to run on-device.
...
- [`../onJetson/README.md`](../onJetson/README.md) — Jetson-side
  container, TensorRT export procedure, and the FastAPI `/predict`
  service that this folder posts to.
